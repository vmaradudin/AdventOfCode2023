...
module Day13 =
    ...
    let parse (input:string array) =
        ...
        |> _.Split($"{Environment.NewLine}{Environment.NewLine}")
        |> Array.map _.Split(Environment.NewLine)

    let checkmirror (lineNum:int) (array:string array) =
        let pairs = 
            [|for i in 0..lineNum -> (lineNum - i - 1, lineNum + i ) |] 
            |> Array.filter (fun (a,b) -> a >= 0 && b<array.Length)
        pairs 
        |> Array.forall (fun (a,b) -> array[a] = array[b])
